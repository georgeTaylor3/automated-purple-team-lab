# variables.pkr.hcl
#
# Same variable names as windows-workstation/variables.pkr.hcl, so both
# pipelines stay easy to compare. Real values live in an ignored
# .pkrvars.hcl file, not here.
...
variable "zone" {
  description = "Google Cloud zone in which Packer creates the temporary Ubuntu builder."
  type        = string
...
variable "packer_builder_service_account" {
  description = "Runtime service account attached to the temporary Ubuntu builder VM."
  type        = string
}

variable "machine_type" {
  description = "Compute Engine machine type used only while constructing the Ubuntu image."
  type        = string
  default     = "e2-medium"
}

variable "disk_size" {
  description = "Temporary Ubuntu boot disk size in GiB."
  type        = number
  default     = 20
}
