...
resource "google_compute_firewall" "allow_control_node_http_egress" {
  name        = "allow-control-node-http-egress"
  description = "Allow control-node outbound HTTP for the GCE regional apt mirror, which does not support HTTPS. Needed for Docker builds run on control-node itself (e.g. the CALDERA image), not just temporary Packer builders."
